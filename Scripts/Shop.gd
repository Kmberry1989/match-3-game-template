...
func _make_frame_card(frame_id: String) -> Control:
	...
	var display_path: String = "res://Assets/Visuals/" + String(data["display"]) # already a filename
	...
	# Status badge moved below the image so the frame is fully visible
	...
	var owned: bool = frame_id in (PlayerManager.player_data.get("unlocks", {}).get("frames", []) as Array)
	if owned:
...
func _refresh():
	...
	# Rebuild cards in the horizontal list
	_frame_ids = _get_sorted_frame_ids()
