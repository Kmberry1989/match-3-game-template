...
func play_idle_animation():
	...
	if animation_state == "idle": # Make sure we weren't interrupted
		...
		if animation_state == "idle": # Check again before returning
			var return_tween = get_tree().create_tween()
			# Deflate and return to position
			return_tween.parallel().tween_property(self, "position", original_pos, 0.5).set_trans(Tween.TRANS_BOUNCE).set_ease(Tween.EASE_OUT)
			return_tween.parallel().tween_property(sprite, "scale", PULSE_SCALE_MIN, 0.5).set_trans(Tween.TRANS_BOUNCE).set_ease(Tween.EASE_OUT)
			return_tween.parallel().tween_property(shadow, "scale", original_shadow_scale, 0.5).set_trans(Tween.TRANS_BOUNCE).set_ease(Tween.EASE_OUT)
			return_tween.parallel().tween_property(shadow, "modulate:a", original_shadow_opacity, 0.5).set_trans(Tween.TRANS_BOUNCE).set_ease(Tween.EASE_OUT)
			await return_tween.finished
			
			set_normal_texture()
			start_pulsing()

func play_match_animation(start_delay):
	animation_state = "surprised"
	if pulse_tween:
		pulse_tween.kill()
	...
	flash.scale = Vector2(0,0)
	add_child(flash)
	var flash_tween = get_tree().create_tween()
	flash_tween.tween_property(flash, "scale", Vector2(1,1), 0.15).set_ease(Tween.EASE_OUT)
	flash_tween.tween_property(flash, "scale", Vector2(0,0), 0.2).set_ease(Tween.EASE_IN)
	...
	rise_tween.parallel().tween_property(self, "position", original_pos + Vector2(0, -20), 0.2).set_ease(Tween.EASE_OUT)
	rise_tween.parallel().tween_property(shadow, "scale", original_shadow_scale * 1.5, 0.2).set_ease(Tween.EASE_OUT)
	rise_tween.parallel().tween_property(shadow, "modulate:a", original_shadow_opacity * 0.7, 0.2).set_ease(Tween.EASE_OUT)
	await rise_tween.finished
	
	# 2. Jitter
	var jitter_tween = get_tree().create_tween().set_loops(5)
	jitter_tween.tween_property(self, "position:x", self.position.x + 3, 0.05)
	jitter_tween.tween_property(self, "position:x", self.position.x - 3, 0.05)
	await get_tree().create_timer(0.5).timeout
	jitter_tween.kill()
	self.position.x = original_pos.x # Reset jitter
	
	# 3. Poof
	var poof_tween = get_tree().create_tween()
	poof_tween.parallel().tween_property(self, "position", self.position + Vector2(0, -70), 0.3).set_ease(Tween.EASE_IN)
	poof_tween.parallel().tween_property(self, "modulate:a", 0.0, 0.3)
	poof_tween.parallel().tween_property(shadow, "scale", original_shadow_scale * 3.0, 0.3).set_ease(Tween.EASE_IN)
	poof_tween.parallel().tween_property(shadow, "modulate:a", 0.0, 0.3).set_ease(Tween.EASE_IN)
	
	await poof_tween.finished
...
func play_sad_animation():
	...
	if animation_state == "sad":
		set_normal_texture()

func play_drag_sad_animation():
	if animation_state == "normal" or animation_state == "surprised":
		animation_state = "sad"
		sprite.texture = sad_texture
...
func move(target):
	var tween = get_tree().create_tween()
	# Use a bounce transition for a more playful feel
	tween.tween_property(self, 'position', target, 0.4).set_trans(Tween.TRANS_BOUNCE).set_ease(Tween.EASE_OUT)
	return tween # Return the tween to be awaited

func dim():
	# This function is now only used for debugging or if an animation fails.
	# The primary feedback is the texture change.
	sprite.modulate = Color(1, 1, 1, 0.8)

func start_floating():
	var float_tween = get_tree().create_tween().set_loops(-1)
	...
	var sprite_start_y = sprite.position.y
	var shadow_start_y = shadow.position.y

	# Float down
	float_tween.tween_property(sprite, "position:y", sprite_start_y + float_amount, float_duration)
	# Float up
	float_tween.tween_property(sprite, "position:y", sprite_start_y, float_duration)

	var shadow_tween = get_tree().create_tween().set_loops(-1)
	shadow_tween.set_trans(Tween.TRANS_SINE)
	shadow_tween.set_ease(Tween.EASE_IN_OUT)
	shadow_tween.tween_property(shadow, "position:y", shadow_start_y + float_amount, float_duration)
	shadow_tween.tween_property(shadow, "position:y", shadow_start_y, float_duration)

func start_pulsing():
	if pulse_tween:
		pulse_tween.kill()
	
	pulse_tween = get_tree().create_tween().set_loops(-1)
	pulse_tween.tween_property(sprite, "scale", PULSE_SCALE_MAX, 1.0)
	pulse_tween.tween_property(sprite, "scale", PULSE_SCALE_MIN, 1.0)
